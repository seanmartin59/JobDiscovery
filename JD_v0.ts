...
function gate0_writeTestRows() {
  ...
  function rolesHasUrl_(rolesSheet, canonicalUrl) {
    ...
    const values = rolesSheet.getRange(2, 1, lastRow - 1, 1).getValues().flat();
  ...
  function getColumnValues_(sheet, colIndex) {
    ...
    if (lastRow < 2) return [];
    return sheet.getRange(2, colIndex, lastRow - 1, 1).getValues().flat().filter(Boolean);
  }
  ...
  function gate3B_enrichNewRoles_fetchJD() {
    ...
    const data = roles.getRange(2,1,lastRow-1,roles.getLastColumn()).getValues();
